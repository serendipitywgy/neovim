...
set_keymaps("n", { "<leader>fc" }, function() Snacks.picker.files({ cwd = vim.fn.stdpath("config") }) end,     { desc = "Find Config File" })
...
set_keymaps({ "n", "x" }, { "<leader>sw" }, function() Snacks.picker.grep_word() end, { desc = "Visual selection or word" })
...
set_keymaps("n", { "<leader>sS" }, function() Snacks.picker.lsp_workspace_symbols() end, { desc = "LSP Workspace Symbols" })
...
set_keymaps({ "n", "x", "o" }, {"S"},  function() require("which-key").show({ global = false }) end, {desc = "Buffer Local Keymaps (which-key)"})
